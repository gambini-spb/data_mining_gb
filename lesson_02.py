...
class GbBlogParse:
    ...
    def parse_post(self, response: requests.Response):
        ...
        data = {
            "post_data": {
                ...
                "image_link": soup.find("img").attrs.get("src"),
                "time": soup.find("time", attrs={"class": "text-md text-muted m-r-md"}).attrs.get("datetime")
            },
            "author_data": {
    ...
    def _get_comments(self, post_id):
        ...
        current_url = urljoin(self.start_url, api_path)
        response = self._get_response(current_url)
        data: list = []
        for thread in response.json():
            data.append({"comment_author": thread["comment"]["user"]["full_name"],
                         "comment_text": thread["comment"]["body"]
                         })
            # Глубина парсера комментариев = 2, иначе необходима рекурсия
            if thread["comment"]["children"]:
                for child in thread["comment"]["children"]:
                    data.append({"comment_author": child["comment"]["user"]["full_name"],
                                 "comment_text": child["comment"]["body"]
                                 })
        return data
